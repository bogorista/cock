    ...
    foreach ($expenses as $value):
      ...
      <td class="hidden-xs"><span><?php $unix = human_to_unix($value->date.' 00:00'); echo '<span class="hidden">'.$unix.'</span> '; echo date($core_settings->date_format, $unix);?></span></td>
			<td class="hidden-xs minus "><span class="<?php if(is_object($value->invoice)){echo 'plus tt" title="'.$this->lang->line("application_rebilled_on_invoice").' #'.$value->invoice->reference; }?>"><?=display_money(sprintf("%01.2f", round($value->value, 2)));?></span></td>
