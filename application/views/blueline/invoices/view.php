<div class="row">
			...
			<?php if($invoice->status != "Paid" && is_object($invoice->company->client)){ ?><a href="<?=base_url()?>invoices/sendinvoice/<?=$invoice->id;?>" class="btn btn-primary"><i class="fa fa-envelope visible-xs"></i><span class="hidden-xs"><?=$this->lang->line('application_send_invoice_to_client');?></span></a><?php } ?>
		...
		<?php if($invoice->estimate_status != "Invoiced"){ ?>
			...
			<td class="hidden-xs"><?=nl2br($invoice->invoice_has_items[$i]->description);?></td>
